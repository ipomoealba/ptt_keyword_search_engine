...
from .models import UserSavePost, Gossiping, GossipingContent, GossipingReply, Newkeywords, UserAnalysisResult
from .utilities import Tracking_concept_drift
...
def home(request):
    ...
    if request.POST:
        ...
        process_month = subprocess.Popen(
            ['python3', code_file, str(all_month_date_freq), filename, "month"], stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        out1, err1 = process_month.communicate()
        process_week = subprocess.Popen(
            ['python3', code_file, str(all_week_date_freq), filename, "week"], stdout=subprocess.PIPE,
            stderr=subprocess.PIPE)
        ...
        for i in raw_data:
            ...
            for ck in content_keywords[2:len(content_keywords) - 2].split('], ['):
                ...
                elif ckeyword[0] != '' and len(ckeyword[0]) > 3 and ckeyword[0] not in stopwords:
                    content_result[ckeyword[0]] = int(ckeyword[1])
            ...
            for rk in reply_keywords[2:len(reply_keywords) - 2].split('], ['):
                ...
                elif rkeyword[0] != '' and len(rkeyword[0]) > 3 and rkeyword[0] not in stopwords:
                    reply_result[rkeyword[0]] = int(rkeyword[1])
                ...
                for i, w in enumerate(tmp):
                    ...
                    for i in w.split('", "'):
                        if "push_content" in i:
                            print()
                            tmp_result[2] = i[len("push_content")+1:].replace("\"", "").replace(":", "").strip()
                        if "push_tag" in i:
                            print(i[len("push_tag")+2: ].replace("\"", "").replace(":", "").strip())
                            tmp_result[0] = i[len("push_tag")+2: ].replace("\"", "").replace(":", "").strip()
                        if "push_user" in i:
                            print(i[len("push_user")+1: ].replace("\"", "").replace(":", "").strip())
                            tmp_result[1] = i[len("push_user")+1: ].replace("\"", "").replace(":", "").strip()
                        if "push_date" in i:
                            print(i[len("push_date")+1: ].replace("\"", "").replace(":", "").strip())
                            tmp_result[3] = i[len("push_date")+1: ].replace("\"", "").replace(":", "").strip()
                    reply.append(tmp_result)
...
def analysis(request):
    ...
    interval = int(request.GET.get('interval') or 5)

    ranking = int(request.GET.get('ranking') or 10)
    print(class_id, user_id, interval, ranking)
    if class_id and user_id and interval:
        query_back = UserAnalysisResult.objects.filter(class_id=class_id, user_id=user_id, other=interval).count()
        if query_back == 0:
            user_save_post = UserSavePost.objects.filter(
                class_id=class_id, user_id=user_id)
            data = Gossiping.objects.filter(
                pid__in=user_save_post.values('pid')).order_by('ptime')

            tcd = Tracking_concept_drift(data)

            result = tcd.cal_concept_drift_score_date_list(interval)

            result_model = UserAnalysisResult(pickle=tcd.pickle_encoder_with_base64(result), dataType="pickle", class_id=class_id, user_id=user_id, other=interval)    
            result_model.save()

    elif not class_id and not user_id:
        payload = {'success': False}
        return HttpResponse(status=404)
    query_back = UserAnalysisResult.objects.filter(class_id=class_id, user_id=user_id)
    tcd = Tracking_concept_drift()
    results = [] 
    for row in query_back:           
        result = tcd.pickle_decoder_with_base64(row.pickle)

                # print(result[1][_1][_2])
        # print((result[1][0][0][:10]))
        # print((result[1][0][1][:10]))
        # print((result[1][0][2][:10]))
        for _1, interval_data in enumerate(result[1]):
            for _2, k in enumerate(interval_data):
                result[1][_1][_2] = k[:ranking]
        # print(result)
        # print("\n\n\n\n\n")
        results.append(result)
    # print(results)
    return render(request, 'analysis.html', {'results': results,
                                             'class_id': class_id})
